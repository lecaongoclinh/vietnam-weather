...
def get_hourly_weather(city, lat, lon, date):
    """
    city: tên tỉnh
    date: datetime object (ngày muốn lấy dữ liệu)
    """
...
def remove_empty_rows(input_csv, output_csv=None):
    """
    Xử lý hàng trống trong file CSV.
    
    input_csv: đường dẫn file CSV gốc
    output_csv: đường dẫn file CSV đầu ra (nếu None sẽ ghi đè file gốc)
    """
    # Đọc file CSV
    df = pd.read_csv(input_csv, encoding='utf-8-sig')
    
    # Xóa các hàng toàn bộ trống
    df_clean = df.dropna(how='all')
    
    # Lưu file
    if output_csv is None:
        output_csv = input_csv  # ghi đè lên file gốc
    
    df_clean.to_csv(output_csv, index=False, encoding='utf-8-sig')
    print(f"✅ Xử lý xong. File sạch đã lưu: {output_csv}")
    
    return df_clean

remove_empty_rows(output_file)